...
function Terrain(){
	...
	this.render_tiles = function(tilemap, tilemap_index){
		...
		for(var i in tilemap){
			...
			if( !adjacent_tiles.north_match && !adjacent_tiles.west_match ){
				...
				chunk_geo.faces.push( new THREE.Face3( vertex_index, vertex_index+1, vertex_index+2 ) );
				chunk_geo.faces.push( new THREE.Face3( vertex_index+2, vertex_index+1, vertex_index+3 ) );
				...
				var tmp_vertex_map = [vertex_map[i - cube_size][1], vertex_index, vertex_map[i - cube_size][3], vertex_index + 1];
				chunk_geo.faces.push( new THREE.Face3( tmp_vertex_map[0], tmp_vertex_map[1], tmp_vertex_map[2] ) );
				chunk_geo.faces.push( new THREE.Face3( tmp_vertex_map[2], tmp_vertex_map[1], tmp_vertex_map[3] ) );
				...
				var tmp_vertex_map = [vertex_map[(i*1)-1][2], vertex_map[(i*1)-1][3], vertex_map[i - cube_size][3], vertex_index];
				chunk_geo.faces.push( new THREE.Face3( tmp_vertex_map[0], tmp_vertex_map[1], tmp_vertex_map[2] ) );
				chunk_geo.faces.push( new THREE.Face3( tmp_vertex_map[2], tmp_vertex_map[1], tmp_vertex_map[3] ) );
		...
		for(var i in tilemap){
			...
			if( adjacent_tiles.south_side_greater || adjacent_tiles.south_side_smaller ){
				chunk_geo.faces.push( new THREE.Face3( vertex_map[i][1], vertex_map[(i*1)+cube_size][0], vertex_map[i][3] ) );
				chunk_geo.faces.push( new THREE.Face3( vertex_map[(i*1)+cube_size][0], vertex_map[(i*1)+cube_size][2], vertex_map[i][3] ) );
	...
	this.get_adjacent_tops = function(tilemap, tile_index, height, x, z, occlusion_map){
		...
		if( z > 0 ){
			var north_height = (Math.round(tilemap[(tile_index*1)-cube_size].height * 10) * self.tile_width) - 300;
