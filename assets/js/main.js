...
function Terrain(){
	...
	this.buffer_builder = new Worker('assets/workers/buffer_builder.js');

	this.buffer_builder.addEventListener('message', function(e) {

		if( typeof(e.data.uvs) == 'object' && typeof(e.data.vertices) == 'object' ){
			self.render_tiles( e.data );
		} else {
	...
	this.build_chunk = function(tilemap, tilemap_index){

		self.buffer_builder.postMessage({
			tilemap: tilemap,
			cube_size: cube_size,
			adjusted_cube_size: adjusted_cube_size,
	...
	this.render_tiles = function(tile_data){

		var vertices = tile_data.vertices;
		var uvs = tile_data.uvs;
		var tilemap_index = tile_data.tilemap_index;

		var chunk_geometry = new THREE.BufferGeometry();
		...
		chunk_geometry.computeVertexNormals();

		var chunk = new THREE.Mesh( chunk_geometry, self.atlas_tex );
		...
		origin_x -= (cube_size*self.tile_width/2);
		origin_z -= (cube_size*self.tile_width/2);
		...
		self.tilemaps[tilemap_index] = chunk;

		if( tilemap_index == origin_point ){
			scene.add( self.tilemaps[tilemap_index] );
		}
	}
	...
	this.mouse_listeners = function(){
		...
		$(renderer.domElement).on('mousewheel', function(event) {
			if(Math.abs(event.deltaY) >= 3){
				...
				self.camera_zoom = self.camera_zoom > 4 ? 4 : self.camera_zoom;
