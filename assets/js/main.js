...
function Terrain(){
	...
	(function init_assets(){
		...
		var materials = [
			...
			new THREE.MeshLambertMaterial( { color: self.brown } )
		];
	...
	this.render_tiles = function(tilemap, tilemap_index){
		...
		for(var i in tilemap){
			...
			if( adjacent_tiles.south_side_greater || adjacent_tiles.south_side_smaller ){
				chunk_geo.faces[(i*2)+1].materialIndex = 3;
			}
		...
		var chunk_sides = new THREE.Mesh(chunk_geo_sides, self.terrain_texture );
		...
		chunk_sides.position.x = origin_x;
		chunk_sides.position.z = origin_z;
		chunk_sides.position.x *= 1.01;
		chunk_sides.position.z *= 1.01;
scene.add(chunk_sides)
		chunk.is_chunk = true;
